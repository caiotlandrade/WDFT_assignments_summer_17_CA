...
// setting EJS as the templating engine for Express
app.set('view engine', 'ejs');

// setting the routing for index page
app.get('/', (req, res) => {
  res.render('pages/index', {movies: getmovies()});
});

// setting the routing for the dynamic URL
app.get('/movie/:movieID', (req, res) => {
  ...
  let moviesArray = getmovies();
    ...
    for (var i = 0; i < moviesArray.length; i++) {
        if (movie === moviesArray[i].id) {
            return res.render('pages/movie', {myMovie: moviesArray[i]});
        }
    }
    res.send("This is not a valid user.");
});

// setting the routing for the search query
app.get('/search', (req, res) => {
  let movieSearch = req.query.searchText;
  ...
  for(let i=0; i<moviesArray.length; i++) {
    if(   moviesArray[i].title.toLowerCase().includes(movieSearch.toLowerCase())
       || moviesArray[i].genre.toLowerCase().includes(movieSearch.toLowerCase())
       || moviesArray[i].director.toLowerCase().includes(movieSearch.toLowerCase())
       || moviesArray[i].writer.toLowerCase().includes(movieSearch.toLowerCase())
       || moviesArray[i].actors.toLowerCase().includes(movieSearch.toLowerCase())
       || moviesArray[i].plot.toLowerCase().includes(movieSearch.toLowerCase())
    ) {
